...
function _runOnDirs_getDirs ({root = "."} = {}) {
	return fs.readdirSync(root, "utf8")
		.filter(dir => isDirectory(path.join(root, dir)) && !dir.startsWith(".") && !dir.startsWith("_") && dir !== "node_modules");
}

function runOnDirs (fn, {root = "."} = {}) {
	_runOnDirs_getDirs({root})
		.forEach(dir => fn(dir));
}

async function pRunOnDirs (pFn, {root = ".", isSerial = false} = {}) {
	const dirs = _runOnDirs_getDirs({root});
